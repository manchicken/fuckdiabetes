...
is($one_loaded->author, 'Xabc123', 'Verify the author name came through...');
is($one_loaded->author_oid, 'abc123abc123', 'Verify the author_oid came through...');

my $byauthor_obj = FuckDiabetes::Data::Brag->find_all_by_author('Xabc123');
...
my $t1 = FuckDiabetes::Data::User->new(access_token=>'Xabc123', name=>'Xsomebody');
isa_ok($t1, q{FuckDiabetes::Data::User}, 'Verify we got a proper instance...');
is($t1->access_token, 'Xabc123', 'Verify we got the access token we put in...');
is($t1->name, 'Xsomebody', 'Verify we got the name we put in...');
ok($t1->save(), 'Verify we can save...');

$one_loaded->author_oid_from_object($t1);
is($one_loaded->author_oid, $t1->oid(), 'Verify we properly set the author_oid from an object...');
